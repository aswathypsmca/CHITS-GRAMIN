...
if (!empty($_POST["Did"])) {
 ...
 // $cid = $_POST["c_id"];
  ...
  $tamount=$row1['chitt_amount'];
   ...
   $sql3="SELECT * FROM `tbl_auction` WHERE `c_id`='$cid' and `a_id` in (SELECT max(`a_id`) FROM `tbl_auction`)";
   $results3 = mysqli_query($con, $sql3);
   $row3=mysqli_fetch_array($results3);
   $auc_amount=$row3['amount'];
   $au_am=$auc_amount-$comm;
  ...
  "; 
	...
	{
		 $install_sec=($auc_amount+$tamount)/$div;
	}
 // SELECT * FROM `tbl_chittypayment` WHERE `cust_ch_id`='79' and `cid`='12' and  `p_id`=(SELECT MAX(`p_id`) FROM `tbl_chittypayment` WHERE `cust_ch_id`='79' and `cid`='12')
  ...
  echo  $row['c_name'] .":" . $row['c_address'] . ":" .$row['c_mob'] . ":" .$install_sec.":" .$new_ino.":" .$cid;
